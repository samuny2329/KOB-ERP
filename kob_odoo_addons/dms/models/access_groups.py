...
class DmsAccessGroups(models.Model):
    ...
    @api.depends(
        "parent_group_id",
        "parent_group_id.users",       # self-ref M2M = "users"
        "group_ids",
        "group_ids.user_ids",          # Odoo 19: res.groups.user_ids
        "explicit_user_ids",
    )
    def _compute_users(self):
        # Odoo 19: res.groups.users renamed to user_ids on res.groups,
        # but dms.access.group.users (self) and parent.users still exist.
        for record in self:
            users = (
