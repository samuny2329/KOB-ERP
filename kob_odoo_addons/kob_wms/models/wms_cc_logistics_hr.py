...
class WmsCcHrTurnover(models.Model):
    ...
    def init(self):
        # Odoo 19 moved hr.employee.department_id onto hr.version (employee
        # versioning).  Join hr_employee → hr_version on current_version_id
        # to get the active department/company per employee.
        tools.drop_view_if_exists(self.env.cr, self._table)
        self.env.cr.execute("""
            CREATE OR REPLACE VIEW %s AS (
                SELECT
                    ROW_NUMBER() OVER (ORDER BY v.department_id) AS id,
                    v.company_id,
                    v.department_id,
                ...
                GROUP BY v.company_id, v.department_id
            )
        """ % self._table)
